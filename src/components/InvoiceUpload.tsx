...
export default function InvoiceUpload({ targetType, fixedTercero, onSuccess }: InvoiceUploadProps) {
    ...
    const handleCreateTercero = async (id: string) => {
        const item = uploadQueue.find(i => i.id === id);
        if (!item || !item.extractedData?.rut || !item.extractedData?.tercero_nombre) {
            alert("Faltan datos del tercero (RUT o Razón Social).");
            return;
        }

        const isProvider = targetType === 'proveedor' || (item.extractedData.tipo === 'compra');

        if (!isProvider && (!item.extractedData.email || !item.extractedData.telefono)) {
            alert("El Correo Electrónico y el Teléfono son obligatorios para crear un nuevo cliente.");
            return;
        }

        const cleanRut = item.extractedData.rut.replace(/\./g, '').replace(/-/g, '').toUpperCase();
        setIsSaving(true);

        try {
            const payload: any = {
                rut: cleanRut,
                razon_social: item.extractedData.tercero_nombre,
                email: item.extractedData.email,
                telefono: item.extractedData.telefono,
                tipo: item.extractedData.tipo === 'venta' ? 'cliente' : 'proveedor',
                estado: 'activo'
            };
            ...
            if (error) {
                if (error.message?.includes('plazo_pago_dias') || error.code === 'PGRST204') {
                    console.warn("Reintentando creación sin plazo_pago_dias...");
                    ...
                    setUploadQueue(prev => prev.map(i =>
                        i.id === id ? { ...i, terceroEncontrado: data2 } : i
                    ));
                } else {
                    console.error("Error de Supabase al insertar tercero:", error);
                ...
                setUploadQueue(prev => prev.map(i =>
                    i.id === id ? { ...i, terceroEncontrado: data } : i
                ));
            }
            alert("Tercero creado correctamente.");
        } catch (error: any) {
            console.error("Excepción al crear tercero:", error);
            alert(`Error al crear tercero: ${error.message}`);
        } finally {
            setIsSaving(false);
        }
    };
